...
namespace ZeroGUI
{
	namespace Colors
	{
		...
		FLinearColor Window_Background{ 0.11f, 0.15f, 0.17f, 1.00f };
		FLinearColor Window_Header{ 0.11f, 0.15f, 0.17f, 1.00f };

		FLinearColor Button_Idle{ 0.20f, 0.25f, 0.29f, 1.00f };
		FLinearColor Button_Hovered{ 0.28f, 0.56f, 1.00f, 1.00f };
		FLinearColor Button_Active{ 0.06f, 0.53f, 0.98f, 1.00f };

		FLinearColor Checkbox_Idle{ 0.17f, 0.16f, 0.23f, 1.0f };
		FLinearColor Checkbox_Hovered{ 0.22f, 0.30f, 0.72f, 1.0f };
		FLinearColor Checkbox_Enabled{ 0.28f, 0.56f, 1.00f, 1.00f };
	...
	void SetupCanvas(UObject* _canvas)
	{
		canvas = _canvas;
	}
	...
	void TextLeft(char* name, FVector2D pos, FLinearColor color, bool outline)
	{
		...
		K2_DrawText(canvas, GetFont(), FString{ s2wc(name) }, pos, FVector2D{ 0.97f, 0.97f }, color, false, Colors::Text_Shadow, FVector2D{ pos.X + 1, pos.Y + 1 }, false, true, true, Colors::Text_Outline);
	}
	void TextCenter(char* name, FVector2D pos, FLinearColor color, bool outline)
	{
		int length = strlen(name) + 1;
		K2_DrawText(canvas, GetFont(), FString{ s2wc(name) }, pos, FVector2D{ 0.97f, 0.97f }, color, false, Colors::Text_Shadow, FVector2D{ pos.X + 1, pos.Y + 1 }, true, true, true, Colors::Text_Outline);
	}
	...
	void drawFilledRectA(FVector2D initial_pos, float w, float h, FLinearColor color)
	{
	...
	bool Window(char* name, FVector2D* pos, FVector2D size, bool isOpen)
	{
		...
		//Bg
		drawFilledRectA(FVector2D{ pos->X, pos->Y }, size.X, size.Y, Colors::Window_Background);
		//drawFilledRect(FVector2D{ pos->X, pos->Y }, 122, size.Y, FLinearColor{ 0.006f, 0.006f, 0.006f, 1.0f });//My tabs bg

		//Header
		drawFilledRectA(FVector2D{ pos->X, pos->Y }, size.X, 25.0f, Colors::Window_Header);
	...
	bool ButtonTab(char* name, FVector2D size, bool active)
	{
		...
		if (active)
		{
			drawFilledRectA(FVector2D{ pos.X, pos.Y }, size.X, size.Y, Colors::Button_Active);
		}
		else if (isHovered)
		{
			drawFilledRectA(FVector2D{ pos.X, pos.Y }, size.X, size.Y, Colors::Button_Hovered);
			hover_element = true;
		}
		else
		{
			drawFilledRectA(FVector2D{ pos.X, pos.Y }, size.X, size.Y, Colors::Button_Idle);
		}
	...
	void Checkbox(char* name, bool* value)
	{
		...
		if (isHovered)
		{
			drawFilledRectA(FVector2D{ pos.X, pos.Y }, size, size, Colors::Checkbox_Hovered);
			hover_element = true;
		}
		else
		{
			drawFilledRectA(FVector2D{ pos.X, pos.Y }, size, size, Colors::Checkbox_Idle);
		}
		...
		if (*value)
		{
			drawFilledRectA(FVector2D{ pos.X + 3, pos.Y + 3 }, size - 6, size - 6, Colors::Checkbox_Enabled);
	...
	void SliderInt(char* name, int* value, int min, int max)
	{
		...
		if (isHovered || current_element == elements_count)
		{
			...
			drawFilledRectA(FVector2D{ pos.X, pos.Y + slider_size.Y + padding.Y }, slider_size.X, slider_size.Y, Colors::Slider_Hovered);
			drawFilledRectA(FVector2D{ pos.X, pos.Y + slider_size.Y + padding.Y + 5.0f }, 5.0f, 5.0f, Colors::Slider_Progress);

			hover_element = true;
		}
		else
		{
			drawFilledRectA(FVector2D{ pos.X, pos.Y + slider_size.Y + padding.Y }, slider_size.X, slider_size.Y, Colors::Slider_Idle);
			drawFilledRectA(FVector2D{ pos.X, pos.Y + slider_size.Y + padding.Y + 5.0f }, 5.0f, 5.0f, Colors::Slider_Progress);
		}
		...
		drawFilledRectA(FVector2D{ pos.X, pos.Y + slider_size.Y + padding.Y }, oneP * (*value - min), slider_size.Y, Colors::Slider_Progress);
	...
	void Combobox(char* name, FVector2D size, int* value, const char* arg, ...)
	{
		...
		{
			drawFilledRectA(FVector2D{ pos.X, pos.Y }, size.X, size.Y, Colors::Combobox_Hovered);

			hover_element = true;
		}
		else
		{
			drawFilledRectA(FVector2D{ pos.X, pos.Y }, size.X, size.Y, Colors::Combobox_Idle);
		}
	...
	void ColorPicker(char* name, FLinearColor* color)
	{
		...
		else
		{
			...
			//Color
			drawFilledRectA(FVector2D{ pos.X + 4, pos.Y + 4 }, size - 8, size - 8, *color);
	...
	void Render()
	{
		for (int i = 0; i < 128; i++)
		{
			if (PostRenderer::drawlist[i].type != -1)
			{
				...
				if (PostRenderer::drawlist[i].type == 1)
				{
					ZeroGUI::drawFilledRectA(PostRenderer::drawlist[i].pos, PostRenderer::drawlist[i].size.X, PostRenderer::drawlist[i].size.Y, PostRenderer::drawlist[i].color);
				}
				//TextLeft
