...
public:
	...
	static inline TUObjectArray GetGlobalObjects()
	{
		...
		if (!GObjects)
			GObjects = (FUObjectArray*)((DWORD64)GetModuleHandleW(NULL) + 0x9983A70);
...
{
	auto ss = FindPattern(E("48 8B 0D ? ? ? ? 48 98 4C 8B 04 D1 48 8D 0C 40 49 8D 04 C8 EB ?"));
...
void Render()
{
			...
			for (int i = 0; i < actors.Num(); i++)
			{
				...
				else if (levelIndex == 0 && Object_IsA(actor, SC_FortPlayerPawn()))
				{
					...
					if (W2S(loc, sp))
					{
						...
						{
							xDrawText(E(L"Enemy Visable"), { (float)(g_ScreenWidth / 2) , (float)(g_ScreenHeight - 180) }, Colors::Red);
							bEnemyClose = true;
						}
	...
	{
		sprintf(tier_data, E("Enemy Closest: %d m"), closestEnemyDist);
		AnsiToWide(tier_data, tier_data_wide);
...
uintptr_t GetWorld()
{
	return *(uint64_t*)(GFnBase + 0x9B6B2B0);
}
	...
	if (GWorld && GCanvas)
	{
		...
		if (GPlayerCameraManager)
		{
			...
			if (get_camera(view, GPlayerCameraManager))
			{
				...
				if (G::FovChanger)
				{
					FOV((UObject*)GController, G::FOVSIZE);
				}
...
void HkPostRender(UObject* _this, UObject* canvas)
{
	...
	if (GCanvas && G::ShowTimeConsumed)
	{
		...
		sprintf(time_buff, E("PostRender: %d"), delta);
...
void drawFilledRectQ(const FVector2D& initial_pos, float w, float h, const FLinearColor& color);
...
{
	drawFilledRectQ(initial_pos, w, h, color);
	UControl bounds;
...
void MenuDrawTabs()
{
	...
	sprintf(logo_buff, E("FIXXED VERSION FOR MICCA UHHH"), E(__DATE__), E(__TIME__));
...
void MenuCheckbox(FVector2D sp, const wchar_t* text, bool* shittobind)
{
	...
	drawFilledRectQ(sp + g_Clientarea + 3, 9, 9, color);
...
void Tick()
{
	...
	if (ZeroGUI::Window((char*)"Evolution Reborn - Toggle Menu - F1", &ZeroGUI::menu_pos, FVector2D{ 450.0f, 300.0f }, g_Menu))
	{
		//Simple Tabs
		static int tab = 0;
		if (ZeroGUI::ButtonTab((char*)"aim", FVector2D{ 110, 25 }, tab == 0)) tab = 0;
		if (ZeroGUI::ButtonTab((char*)"esp", FVector2D{ 110, 25 }, tab == 1)) tab = 1;
		if (ZeroGUI::ButtonTab((char*)"rage", FVector2D{ 110, 25 }, tab == 2)) tab = 2;
		ZeroGUI::NextColumn(130.0f);

		if (tab == 0)
		{
			ZeroGUI::Checkbox((char*)"Memory", &G::AimbotEnable);
			ZeroGUI::Checkbox((char*)"Silent", &G::Baim);

			ZeroGUI::SliderFloat((char*)"FOV Radius", &G::FOVRadius, 0.0f, 150.0f);
			ZeroGUI::SliderFloat((char*)"Smooth", &G::Smooth, 0.0f, 1.0f);
		}

		if (tab == 1)
		{
			ZeroGUI::Checkbox((char*)"Weapons", &G::EspWeapon);
			ZeroGUI::Checkbox((char*)"Name", &G::EspPlayerName);
			ZeroGUI::Checkbox((char*)"Skeleton", &G::Skeletons);
			ZeroGUI::Checkbox((char*)"2D Box", &G::EspBox);
			ZeroGUI::Checkbox((char*)"Corner", &G::CornerBox);
			ZeroGUI::Checkbox((char*)"Local Player", &G::DrawSelf);
		...
		if (tab == 2)
		{
			ZeroGUI::Checkbox((char*)"Radar", &G::Radar);
			ZeroGUI::Checkbox((char*)"WayppointTP (Enter)", &G::Waypoint);
			ZeroGUI::Checkbox((char*)"Player TP (Knocked)", &G::PlayerTP);
